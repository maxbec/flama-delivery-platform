...
OUTPUT_DIR=${1:-"$ROOT_DIR/build/governance"}

if [[ -e "$OUTPUT_DIR" ]] && find "$OUTPUT_DIR" -mindepth 1 -print -quit | grep -q .; then
  echo 'Governance bundle output directory must be absent or empty' >&2
  exit 1
fi
...
cd "$ROOT_DIR"
pnpm build
pnpm exec ncc build dist/services/governance/src/runtime.js \
  --out "$OUTPUT_DIR" \
  --minify \
  --no-cache \
