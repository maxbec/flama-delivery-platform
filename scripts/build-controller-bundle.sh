...
OUTPUT_DIR=${1:-"$ROOT_DIR/build/controller"}

if [[ -e "$OUTPUT_DIR" ]] && find "$OUTPUT_DIR" -mindepth 1 -print -quit | grep -q .; then
  echo 'Controller bundle output directory must be absent or empty' >&2
  exit 1
fi
...
cd "$ROOT_DIR"
pnpm build
pnpm exec ncc build dist/services/controller/src/main.js \
  --out "$OUTPUT_DIR" \
  --minify \
  --no-cache \
