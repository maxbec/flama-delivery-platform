...
| 4 — Canaries | First canary live; two platform defects already found | A representative five-repository set covering every required dimension is selected from the corrected scope and recorded privately, with its library, Docker, and legacy-stack claims verified by reading the repositories rather than inferred. The first canary is a live pull request that adds the generated Fast-profile delivery files to a public library repository, changing no existing file and enabling no protection. It has already paid for itself: the consumer's own static analysis rejected the generated Dependabot policy for having no cooldown, which lets a freshly published malicious version be proposed immediately, and rejected the generated YAML for having no document start. Both are fixed in the platform. The Flama Policy Gate fails closed on that pull request exactly as designed, because it requires a Paperclip Preflight check that only the still-paused controller can publish. `canary-plan` itself cannot run until `secrets-audit` has a producer, which needs the authoritative Infisical mapping. |
