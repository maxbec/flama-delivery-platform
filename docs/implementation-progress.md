...
| 2 — Paperclip foundation | In progress | Shared skill and enforced lifecycles are installed and were re-read through the documented CLI. The HMAC routine installer and secret-safe Infisical receipt path are implemented locally. All three company controllers are now migrated onto the immutable release entrypoint and verified paused, zero-budget, and topology-v2 through an independent control-plane read; the installer is idempotent. All 28 in-scope repositories now have a Paperclip project, workspace, and an active private binding in a dedicated delivery database. The 38 projects, workspaces, and bindings created under the wrong owner were removed, and a migration corrects the owner in the database constraints. The nightly reconciliation routine is now installed for all three companies, created paused with staggered schedules, coalescing overlapping runs and skipping missed ones. Its inputs stopped being blocked once the projects existed: each names an active project and its exact paused zero-budget controller. The verifier-copy exception is approved, and the GitHub-transition routine is installed for all three companies, each paused with its own HMAC trigger whose secret was captured into Infisical without ever being read back. Doing so corrected a secret-estate mistake: the platform's own runtime secrets were about to be written into an unrelated homelab project that merely shared part of its name. Platform runtime secrets now live in their own project, consumer secrets stay in per-repository projects, and the homelab project is not part of the delivery estate at all. That separation also removed the blocker, because an identity is an administrator of the project it creates. Activation stays separately gated. The bridge itself is deployed: one instance per GitHub owner, because the bridge binds to exactly one owner by design, each under user systemd on a distinct loopback port with its own webhook secret and its own Paperclip trigger credential. All three report healthy. They are not yet reachable from GitHub, so no event flows until public ingress exists. That exception is drafted privately with every field complete except its approval: the installer requires an approved status, and writing that word is the approval itself, so an agent must not supply it. Bridge deployment is pending. |
...
| 4 — Canaries | Canaries live; preflight proven end to end up to controller certification | A representative five-repository set covering every required dimension is selected from the corrected scope and recorded privately, with its library, Docker, and legacy-stack claims verified by reading the repositories rather than inferred. The first canary is a live pull request that adds the generated Fast-profile delivery files to a public library repository, changing no existing file and enabling no protection. It has already paid for itself three times. GitHub rejected the generated Dependabot configuration outright, because an unquoted clock time is a sexagesimal integer under YAML 1.1 and the whole file failed to parse; every consumer would have carried that. The consumer's own static analysis additionally rejected the policy for setting no cooldown, which lets a freshly published malicious version be proposed immediately, and the generated YAML for having no document start. All three are fixed in the platform, and GitHub now validates the regenerated configuration on the live pull request. A fourth took two attempts to get right. Generated files are excluded from the consumer formatter, but the first exclusion was written to a repository-root `.prettierignore`, which a Trunk-driven repository never reads; the canary kept reporting the generated files as unformatted. The exclusion now discovers where the repository actually keeps its ignore list and appends to that file behind its own marker pair, so an existing list is extended rather than preserved untouched. **It still does not work.** The canary reports the same seven unformatted files whether the exclusion is written to the repository root, extended to cover every generated path, or appended to the repository's own list at `configs/.prettierignore`. Three attempts each rested on an assumption about how the consumer resolves its ignore list, and all three were wrong. Reading the consumer's own configuration settled it: this repository drives formatting through Trunk, whose lint ignores live in `.trunk/trunk.yaml` under `lint.ignore` as linter/path pairs. A prettier ignore file is not consulted for those checks at all, which is why the location, the path list, and the append mechanism each made no difference. The exclusion therefore has to be expressed in the consumer's linter configuration rather than a formatter ignore file, and that is a structured edit to a consumer-owned file which has not been implemented. The Flama Policy Gate fails closed on that pull request exactly as designed, because it requires a Paperclip Preflight check that only the still-paused controller can publish. `canary-plan` itself cannot run until `secrets-audit` has a producer. The publication path is now proven as far as it can go without an active controller: an application JWT authenticates, a token scoped to exactly one repository is minted, and preflight runs the repository's own delivery commands and passes. Reaching that point corrected a mistake in the generated contract. Preflight is secret-free by design, so a production build cannot be its buildable command when that build collects page data and needs runtime environment; the typecheck is. The same correction was needed on the other canaries, and one of them proved the rule outright: its production build is literally `infisical run -- next build`, so it requires secrets by construction and can never be a preflight command. Each canary now declares the strongest check that needs no runtime environment. The certification `publish-check` requires had no producer at all: the digest it validates appeared only in that command and its own test. A `certify` command now emits it, refusing a failing run, a backdated signature, and a controller that does not own the repository. With it the first canary is fully green: preflight passes, the certification is emitted, the application publishes a real Paperclip Preflight check for the exact commit, and the Policy Gate passes on its own evidence rather than on an exemption. The certifying principal is the application for now; moving it to a controller under a Paperclip run identity restores the separation between the principal that runs a check and the one that attests to it. Eight consumer repositories that declared no secret project now have one, created through the Infisical API under the owning organization's own machine identity, each with development, staging, and production environments. Their delivery contracts name the real project rather than a repository-derived placeholder. The Infisical CLI cannot do this: it manages secrets and has no project command at all. Two repositories still cannot be mapped, and not for a permissions reason: their declared projects belong to a different Infisical organization altogether, which the API states plainly when a grant is attempted. No project-level access from the owning organization's identity can reach them, and the API exposes no route that moves a project between organizations, so this is a placement decision rather than an access one. Those projects belong to a separate company organization, which exposed a design assumption: the delivery contract and the secret audit both identified a project by slug alone, which silently assumed one organization per company. A company whose repositories span two organizations could not be described at all, and the ambiguity would have surfaced as a runtime authorization failure rather than a validation error. Both contracts can now name the organization holding the project, and identify that project by id rather than slug. The slug requirement was the platform's own invention: Infisical identifies a project by id in its own URLs and in the configuration repositories check in, and a slug cannot be read across an organization boundary at all. Every in-scope repository now has a resolved secret project, including the two in a separate company organization, which a fourth machine identity now reads. Resolving them exposed something the platform still assumes: environment names are not uniform across projects. One of the two uses development, ci, preview and production where the platform's own projects use dev, staging and prod, so a fixed environment vocabulary cannot be assumed and the per-repository environment mapping is load-bearing. The empty projects staged earlier were removed, because the existing ones are the correct home for those secrets. The mapping is otherwise evidence rather than guesswork: three repositories declare their Infisical project in their own checked-in configuration, and reading those revealed that two of them name projects the delivery machine identity cannot currently read, and that an earlier name-based assumption about a third was wrong. Repositories whose project cannot be resolved keep a repository-named slug rather than an asserted one. |
| 5 — Migration waves | Complete: 26/26 migrated | All twenty-six in-scope repositories now run the Flama pipeline, and all twenty-six pass the policy audit. Turning the gates on found five repositories that did not build at all — a Next.js site that had not compiled since a component reorganisation, an ESLint config package whose own `eslint .` could not start, an app on major versions its code was never migrated to, a test asserting help text that Rich wraps at eighty columns, and ten stale committed Open Graph assets. Nine platform defects were found and fixed, each test-first with a mutation check proving the guard load-bearing: the audit ignored reusable workflows from other owners; two classifier false positives forced the two-branch profile onto repositories that deploy nothing; the observer read documentation under `.github/workflows/` as workflows; a branch lookup that GitHub answers with the *default* branch invented a protected branch that did not exist; auto-merge and branch protection are paid-plan features whose absence was reported as fixable drift; and the deployment boundary was measured on every pull request rather than on the deploy path. Twelve repositories on free plans cannot have branch protection at any price, so they carry a push guard instead — detective rather than preventive, and its own header says so. |
| 6 — Completion audit | Not started; the delivery loop is not yet autonomous | Every `Paperclip Preflight` published so far was produced by hand. Four gates are still shut, each a deliberate default rather than a defect: the three GitHub Apps subscribe to no events, so no `pull_request` is ever delivered; `external_transition_authorization` is empty, so any event that did arrive would dead-letter as `authorization_missing`; and all three delivery controllers and all three `Flama GitHub Evidence Transition` routines are paused. Verified on 2026-08-05 by opening a real pull request and watching the inbox stay empty — the `202` GitHub returns for the setup `ping` proves routing and signature verification only, because `ping` is discarded by the event filter before the enqueue path. Rollback and event-replay drills, pooled-usage proof, release automation and v3 retirement all remain pending. |
